...
المخرجات:
- per_class_feature_importance.xlsx (ورقة لكل نموذج + ورقة للمزج الوزني)
...
def main():
    ...
    if args.include_derived:
        ...
        features_to_eval = [c for c in X.columns if c not in DERIVED_COLS]
        features_to_eval = [c for c in features_to_eval if c in feature_cols_training]

    def proba_rf(X_):
        return proba_in_le_order(rf_cal, X_, n_classes, le)
    def proba_gb(X_):
        return proba_in_le_order(gb_cal, X_, n_classes, le)
    def proba_et(X_):
        return proba_in_le_order(et_cal, X_, n_classes, le)
    def proba_stack(X_):
        return proba_in_le_order(stack, X_, n_classes, le)
    def proba_blend(X_):
        return blend_proba(X_, models_for_blend, best_w, n_classes, le)

    results = {}
    results["WeightedEnsemble"] = compute_permutation_importance_per_class(
        X, y_idx, n_classes, proba_blend, n_repeats=args.repeats, features_to_eval=features_to_eval
    )
    if rf_cal is not None:
        results["RandomForest"] = compute_permutation_importance_per_class(
            X, y_idx, n_classes, proba_rf, n_repeats=args.repeats, features_to_eval=features_to_eval
        )
    if gb_cal is not None:
        results["GradientBoosting"] = compute_permutation_importance_per_class(
            X, y_idx, n_classes, proba_gb, n_repeats=args.repeats, features_to_eval=features_to_eval
        )
    if et_cal is not None:
        results["ExtraTrees"] = compute_permutation_importance_per_class(
            X, y_idx, n_classes, proba_et, n_repeats=args.repeats, features_to_eval=features_to_eval
        )
    if stack is not None:
        results["Stacking"] = compute_permutation_importance_per_class(
            X, y_idx, n_classes, proba_stack, n_repeats=args.repeats, features_to_eval=features_to_eval
        )
    ...
    writer.close()
    print(f"✅ تم إنشاء الملف: {args.out}")
