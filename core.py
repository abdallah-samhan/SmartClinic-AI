...
def core_handle_answers(payload):
    ...
    - التوقف عند نفاد الأعراض أو الاكتفاء (max 25 سؤال أو ≥3 إجابات إيجابية)
    ...
    # =========================
    # تحقق من شرط الإيقاف قبل توليد أسئلة M1 جديدة
    # =========================
    stop, reason = m1_is_sufficient(flat_answers)
    if stop:
        logger.info(f"[core_handle_answers] Stopping M1 early: {reason}")
        # تشغيل M2 مباشرة
        m2_result = None
        if top_department:
            disease = DEPT_TO_DISEASE.get(top_department)
            if disease:
                m2_result = run_M2(disease, flat_answers)
    ...
    # =========================
    # لا توجد أعراض جديدة أو نفاد الأعراض → تشغيل M2
    # =========================
    logger.debug("[core_handle_answers] No more symptoms, final M1 scores")
    m2_result = None
    if top_department:
        disease = DEPT_TO_DISEASE.get(top_department)
        if disease:
...
def m1_is_sufficient(answers: dict, max_questions: int = 25, min_positive: int = 3):
    """
    التوقف عند 25 سؤال إذا كان هناك 3 إجابات إيجابية أو أكثر
    """
    if not isinstance(answers, dict):
        return False, "invalid answers"

    # تحويل القيم إلى أرقام إيجابية/سلبية
    def _to_num(v):
        s = str(v).strip().lower()
    ...
    # عدد الإجابات الفعلية
    observed_items = {k: v for k, v in answers.items() if v is not None}
    total_questions = len(observed_items)

    # عدد الإيجابيات
    positive_count = sum(1 for v in observed_items.values() if _to_num(v) > 0)

    # إذا الإيجابيات أقل من الحد الأدنى، لا توقف
    if positive_count < min_positive:
        return False, "not enough positive signals"

    # إذا وصلنا إلى 25 سؤال أو أكثر، توقف دائمًا
    if total_questions >= max_questions:
        return True, "max questions reached with enough positives"
